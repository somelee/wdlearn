...
//	setitimer(ITIMER_REAL,&time,NULL);
	...
	printf("upload begin\n");
	if(fd_total>(2<<20)*100)
	{
		char* p;
		char* s;
		p=(char*)mmap(NULL,fd_total,PROT_READ,MAP_PRIVATE,fd,0);
		s=p+fd_len;
		while(1)
		{
			if(0!=(fd_total-fd_len)/1000)
			{
				memset(&ndata,0,sizeof(ndata));
				ndata.len=1000;
				memcpy(ndata.buf,s,1000);
				send_n(nfd,(char*)&ndata,1008);
				fd_len=fd_len+1000;
				s=s+1000;
			}else{
				memset(&ndata,0,sizeof(ndata));
				ndata.len=(fd_total-fd_len)%1000;
				memcpy(ndata.buf,s,ndata.len);
				send_n(nfd,(char*)&ndata,8+ndata.len);
